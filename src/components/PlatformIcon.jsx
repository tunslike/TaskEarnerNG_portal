    ...
    return (
        <div className='platform-icon' onClick={onSelect}>
            ...
            <h3 className={`${isSelected ? 'text-primaryOrange' : 'text-primaryBlue'}`}>{title}</h3>
                {pricing && 
                    <h4 className='mt-2'>
                        <span className={`${isSelected ? 'bg-primaryOrange text-white' : 'bg-[#efefef] text-primaryBlue'}  px-2 py-1 text-[0.7rem] rounded-[1rem]`}>₦{pricing}</span> <span className='font-[500] text-[0.7rem]'>Per Post</span>
                    </h4>
                }
        </div>
    )
